...
function cariobjek() {
  ...
  if (objek != 0 && city != 0) {
    $.ajax({
      url: "http://localhost/wisatasumbar/cariobjek.php?objek=" + objek + "&city=" + city, data: "", dataType: 'json', success: function (rows) {
        console.log(rows);
...
function simpanpaket() {
  ...
  if (paket_id.length > 0) {
    ...
    if (name == '' || price == '') {
      ...
      $.ajax({
        url: "http://localhost/wisatasumbar/simpanpaket.php?id=" + id + "&id_travel=" + id_travel + "&name=" + name + "&price=" + price, data: "", dataType: 'json', success: function (rows) {
          //console.log(rows);
