...
$connection = mysqli_connect("host=$host dbname=$database user=$user password=$pass");

// Working with PostGIS and Extended-WKB
// ----------------------------

// Using asBinary and GeomFromWKB in PostGIS
$result = mysqli_fetch_all(mysqli_query($connection, "SELECT asBinary($column) as geom FROM $table"));
foreach ($result as $item) {
  $wkb = mysqli_unescape_bytea($item['geom']); // Make sure to unescape the hex blob
  $geom = geoPHP::load($wkb, 'ewkb'); // We now a full geoPHP Geometry object
  
  // Let's insert it back into the database
  $insert_string = mysqli_escape_bytea($geom->out('ewkb'));
  mysqli_query($connection, "INSERT INTO $table ($column) values (GeomFromWKB('$insert_string'))");
}

// Using a direct SELECT and INSERTs in PostGIS without using wrapping functions
$result = mysqli_fetch_all(mysqli_query($connection, "SELECT $column as geom FROM $table"));
foreach ($result as $item) {
  ...
  mysqli_query($connection, "INSERT INTO $table ($column) values ('$insert_string')");
}
```
